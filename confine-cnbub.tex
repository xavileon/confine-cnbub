...
%A CONFINE testbed like Community-Lab consists of at least one CONFINE server (or controller) and a set of CONFINE enabled (testbed) nodes spread over the existing nodes of a community network.
%The controller is a normal computer which must be directly reachable from inside the CN (via CN IPs) and from the internet.
%Nodes are integral parts of the CN and are forming the actual network infrastructure by respecting peering agreements of the CN and relaying other nodes traffic. 
%A CONFINE testbed node implements additional access-control and resource-isolation and management capabiliteis with the objective to grand external researchers/users with a confined acces to the node's processing and network resources.
