...
A CONFINE testbed like Community-Lab consists of at least one CONFINE
\emph{server} (or \emph{controller}) and a set of CONFINE-enabled
(testbed) \emph{nodes} spread among the existing nodes of a community
network.  The controller is a normal computer which must be directly
reachable from inside the CN (via CN IPs) and usually from the
internet.  Nodes are integral parts of the CN and form the actual
network infrastructure by respecting peering agreements of the CN and
relaying other nodes' traffic.  A CONFINE testbed node implements
additional access control, resource isolation and management
capabilities with the objective to grant external researchers/users
with a confined acces to the node's processing and network resources.
...
\item \texttt{confine\_sliver\_deploy} is used to setup the sliver environment
  ...
  syntax as input.
\item \texttt{confine\_sliver\_start/stop/remove} can be used to start, stop, and remove the
