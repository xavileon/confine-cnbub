...
\textbf{Major differences with Community-Lab}. 
...
with the actual community networks (CNs). The research nodes must be deployed among the 
community to be able to perform experiments in a real enviroment with real users. However 
these networks are in fact production networks, so the experiments must have some limitations depending
on the community owners. For instance as general restriction the nodes placed in the middle of the network
cannot execute link layer experiments because it can easily disturb the right operation of the production
environment.
...
\begin{figure*}[!t]
 ...
 \subfloat[][Research devices connected to an existing node (left) and a complete confine cloud with community and research devices (right)]{%
...
buildings, the users normally share a single device placed on the roof (it is not always
possible but it is a good practice).

A typical community deployment in a city, looks like the figure \ref{fig:community_network_simple}.
...
such as WiMax, Ethernet or Optical Fiber.
...
The integration of the Confine devices inside the existing CN requires the collaboration at least
from one part of the community. To make it possible the Confine project should cover some needs or give 
something in exchange. For example new links may be created using the resources of the project.
...
So if a DHCP server exists in the community device (CD), the research device will obtain 
a valid community IP and it will be reachable from the rest of the network and from the Confine server.
Then the experiments between research devices can be performed. If the owner allows it, a WiFi antenna might be 
...
One of the implicit objectives of the Confine project is to enhance the usage of this network model. In 
some cases where the Community needs one or several extra links, as it was explained before, the project can 
contribute in their installation. These will be placed in a community member location,  the adoption 
of a research device (following the integration model before described) will be a requirement.
...
deployed. It implies (as shown in figure \ref{fig:community_network_integration} (right)) the installation of several 
nodes. The community device must be one compatible with the existing CN to be able to connect with. 
The research device can contain one or several WiFi antennas. This characteristic will allow researcher to 
execute any kind of link layer experiments.

If the set of links (between the nodes) are interesting for the community, the users can make use of them. But
knowing that it is an experiment subnetwork, so the the availability and stability cannot be guaranteed.
